...
if constants.debug:
    print("Accuracy before", user.get_accuracy(model_user, instances[0:100]))
user.change_weights(model_user)
if constants.debug:
    print("Accuracy after ", user.get_accuracy(model_user, instances[0:100]))
...
positive_instances, negative_instances, unclassified_instances = user.partition_instances(model_user, classified_instances)
...
# Create user classification rules
...
positive_rules = user.create_rules(explainer_user, positive_instances)
negative_rules = user.create_rules(explainer_user, negative_instances)

if constants.trace:
    print("nb positive rules", len(positive_rules))
    print("nb negative rules", len(negative_rules))
...
sigma = user.create_binary_representation(explainer_user, explainer_AI)
...
for detailed_instance in classified_instances:
    ...
    prediction_user = user.is_classified_by_user(explainer_user.binary_representation)
